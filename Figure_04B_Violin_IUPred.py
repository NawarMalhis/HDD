...
def violin_h_plot(data, labels, positions=None, showmeans=True, title=None,
                  display_means=None, fontsize=22, xlabel='', hh_lines=None, f_name=None,
                  t_name=None):
    # t_name = f"{f_name[0:-4]}_table4.tsv"
    ...
    # for ii in range(len(labels)):
    #     print(f"Average:\t\t{labels[ii]}\t{sum(data[ii]) / len(data[ii]):.4}")
...
if __name__ == '__main__':
    _p = './Data/'
    score_tag = 'IUPred3'  # 'IUPred3', 'LIST-S2'
    labels_list = ['PDB', 'IDR']
    sl_dict = {'_long': [71, 10000], '_short': [0, 71]}
    d_set_dict = {'DBsh': 'PDB', 'CAID23uh': 'binding_protein', 'CAID1uh': 'binding_protein'}
    af_dict = {}
    binding_data_dict = {}
    ...
    for ds in d_set_dict:
        for sl in sl_dict:
            ...
            aff_tag_size(af=af_dict[ds], tag=tag, sz_range=sl_dict[sl])  # filter on binding size
            aff_load_caid_scores(af_dict[ds], f"{_p}scores/", prd_list=[score_tag], merged=False,
                                 remove_missing_scores=True)
            binding_data_dict[ds] = get_tags_list_scores(af_dict[ds], tags=[[tag, '0'], [tag, '1']],
                                                         score_tag=score_tag)
            labels_list.append(f"{ds}{sl}")
            # data.append(binding_data_dict[ds][0])
            data.append(binding_data_dict[ds][1])

    print(len(data), len(labels_list))
    vf_name = f"{_p}results/Figure_4/Right.png"
    t_name = f"{_p}results/Tables/Table_4_Right.tsv"
    violin_h_plot(data=data, labels=labels_list, display_means={'0': 'red', '1': 'green'},
                  xlabel=score_tag, f_name=vf_name, t_name=t_name, fontsize=24)