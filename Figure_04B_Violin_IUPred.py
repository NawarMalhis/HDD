...
if __name__ == "__main__":
    ...
    for ds, tag in d_set_dict.items():
        ...
        for sl_label, sz_range in sl_dict.items():
            af_temp = aff_load3(str(base_path / "af" / f"{ds}.af"))  # fresh copy
            aff_remove_short(af_temp, cut=15)
            aff_tag_size(af_temp, tag=tag, sz_range=sz_range)

            aff_load_caid_scores(
                af_temp, scores_path=f"{base_path}/scores/", prd_list=[score_tag],
                merged=False, remove_missing_scores=True
            )
    ...
    violin_h_plot(
        ...
        xlabel=score_tag,
        title="IUPred3 Scores — Short vs Long Binding Sites",
        f_name=str(base_path / "results" / "Figure_4" / "Figure_4B_right_IUPred.png"),
        t_name=str(base_path / "results" / "Tables" / "Table_4_Right.tsv"),
        fontsize=24,
    )